...
#### HighGrade_Moth: MothED
```{r}
clean= clean %>% mutate(HighGrade_Moth79= ifelse(HighGrade_Moth79==95, NA, HighGrade_Moth79),
...
clean = clean %>% mutate(MomDropout= ifelse(MothED<12, MothED, NA)) %>% mutate(MomDropout= ifelse(is.na(MothED), NA, MomDropout)) %>% mutate(MomHS= ifelse(MothED==12, MothED, NA)) %>% mutate(MomHS=ifelse(is.na(MothED), NA, MothED)) %>% mutate(MomSomeColl= ifelse(MothED>=13 && !is.na(MothED), MothED, NA)) %>% mutate(MomSomeColl= ifelse(is.na(MothED), NA, MomSomeColl))
...
print(round(summary(colreg)$coef[c(2,4),],3))
}
```
