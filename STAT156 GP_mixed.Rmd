...
#### HighGrade_Moth, MomED & Some college
...
#which(colnames(clean)== "HighGrade_Moth79")
#which(colnames(clean)== "HighGrade_Moth102")
#which(colnames(clean)== "HighGrade_Moth104")
clean = clean %>% mutate(MothED=rowMaxs(as.matrix(clean[, c(402:422)])))

table(clean$MothED)
sum(!is.na(clean$MothED))

clean = clean %>% mutate(MomDropout= MothED<12) %>% mutate(MomHS= MothED==12) %>% mutate(MomSomeColl= ifelse(MothED>=13, 1, 0))

table(clean$MomSomeColl)

clean %>% group_by(Race_Child, PreK) %>%
  summarize(avg.MothED = mean(MothED,na.rm = TRUE),sd.MothED = sd(MothED, na.rm = TRUE))

clean %>% group_by(Race_Child, PreK) %>%
  summarize(avg.MothDropout = mean(MomDropout,na.rm = TRUE),sd.MomDropout = sd(MomDropout, na.rm = TRUE))

clean %>% group_by(Race_Child, PreK) %>%
  summarize(avg.MomSomeColl = mean(MomSomeColl,na.rm = TRUE),sd.MomSomeColl = sd(MomSomeColl, na.rm = TRUE))
clean %>% group_by(Race_Child, PreK_FE) %>%
  summarize(avg.MomSomeColl = mean(MomSomeColl,na.rm = TRUE),sd.MomSomeColl = sd(MomSomeColl, na.rm = TRUE))
```


#### AFQT
wE SHOULD DO IMPUTATION BUT I DIDN'T
```{r}
...
AFQT_std_fitted <- lm(data = clean, AgeAFQT_std ~Black+Hispanic+Age_Moth_Birth)$fitted.values

clean <- clean %>%
  mutate(impAFQT_std = ifelse(is.na(AgeAFQT_std), AFQT_std_fitted, AgeAFQT_std))%>%
...
#### Calculate Sample90
```{r}
#Should not use HS_FE90, instead I use PreK_FE
clean <- clean %>% 
  mutate(Sample90 = ifelse((!is.na(PreK_FE)) & 
...
clean <- clean %>%
  ...
  mutate(PreK_FE_3 = ifelse(!is.na(PreK_FE) & HS_3 == 1, 0, PreK_FE))
...
clean= clean %>% 
  ...
  mutate(HOME_Pct_0to3= 
           ifelse(Age2_Yr104<=15 & Age2_Yr104>=14, temp1, HOME_Pct_0to3)) %>%
  mutate(HOME_Pct_0to3 = ifelse(Age2_Yr104>=20 & Age2_Yr104<=21, HOME_Pct86, HOME_Pct_0to3))
...
## Attrit
fereg <- clean %>% 
  filter((!is.na(PreK_FE)) & SampleID!=12 & (Age_Mo104>=19*12 | (DOB_Yr_Child==1985 & DOB_Mo_Child<9))) %>%
...
round(summary(colreg)$coef[c(2:7),],3)
...
v_motherid <- vcovCL(colreg, cluster= ~MotherID, type='HC1')
#round(summary(colreg)$coef[c(2:7,36:37),],3)
colreg_motherid = coeftest(colreg, v_motherid)
...
v_motherid <- vcovCL(colreg, cluster= ~MotherID, type='HC1')
colreg_motherid = coeftest(colreg, v_motherid)
#round(summary(colreg)$coef[c(2:4,8:10),],3)
#round(colreg_motherid[c(2:5,119:128),],3)
round(colreg_motherid[c(2:7),],3)
```
...
clean <- clean %>%
  mutate(tempLD86 = LD86, 
         tempLD86 = ifelse(!is.na(HealthCond86) & (is.na(tempLD86) | tempLD86!=1), 0,tempLD86),
         tempLD88 = LD88, 
         tempLD88 = ifelse(!is.na(HealthCond88) & (is.na(tempLD88) | tempLD88!=1), 0, tempLD88),
         tempLD90 = LD90, 
         tempLD90 = ifelse(!is.na(HealthCond90) & (is.na(tempLD90) | tempLD90!=1), 0, tempLD90),
         tempLD92 = LD92, 
         tempLD92 = ifelse(!is.na(HealthCond92) & (is.na(tempLD92) | tempLD92!=1), 0, tempLD92),
         tempLD94 = LD94, 
         tempLD94 = ifelse(!is.na(HealthCond94) & (is.na(tempLD94) | tempLD94!=1), 0, tempLD94),
         tempLD96 = LD96, 
         tempLD96 = ifelse(!is.na(HealthCond96) & (is.na(tempLD96) | tempLD96!=1), 0, tempLD96),
         tempLD98 = LD98, 
         tempLD98 = ifelse(!is.na(HealthCond98) & (is.na(tempLD98) | tempLD98!=1), 0, tempLD98),
         tempLD100 = LD100, 
         tempLD100 = ifelse(!is.na(HealthCond100) & (is.na(tempLD100) | tempLD100!=1), 0, tempLD100))
...
colnums <- which(colnames(clean) %in% c("HighGrade_Att94","HighGrade_Att96","HighGrade_Att98",                                 "HighGrade_Att100","HighGrade_Att102","HighGrade_Att104"))
for(i in 1:nrow(clean)){
  suppressWarnings(clean[i,colnum] <- max(clean[i,colnums], na.rm = T))
  if(clean[i,colnum] == -Inf){
    clean[i,colnum] <- NA
...
clean <- clean %>%
  ...
  mutate(PoorHealth = ifelse(HealthReport >= 3, 0, PoorHealth))

table(clean$PoorHealth)
```
...
#### Overall
```{r}
outcomes_table5<-c('Repeat','LD','HSGrad','HSGrad_GED','someCollAtt','Idle','Crime','TeenPreg','PoorHealth')
clean <- clean %>% mutate(y = 0)
report <- data.frame()
for(x in outcomes_table5){
colreg <- clean %>%
  mutate(y = clean[[x]]) %>%
  filter(Sample90 == 1) %>%
  lm(y ~
       ...
       factor(Age2_Yr104)+factor(MotherID), .)
v_motherid <- vcovCL(colreg, cluster= ~MotherID, type='HC1')
colreg_motherid = coeftest(colreg, v_motherid)
round(summary(colreg)$coef[c(2,3),],3)
#print(x)
#print(round(colreg_motherid[c(2),],3))
#print(round(summary(colreg)$coef[c(2),],3))
report <- rbind(report,round(summary(colreg)$coef[c(2),],3))
}
report
```
...
#print(x)
report <- rbind(report,round(summary(colreg)$coef[c(2,4),],3))
#report <- rbind(report,round(colreg_motherid[c(2,4),],3))
}
report
```
...
#print(x)
#print(round(colreg_motherid[c(2,4),],3))
report <- rbind(report,round(summary(colreg)$coef[c(2,4),],3))
#report <- rbind(report,round(colreg_motherid[c(2,4),],3))
}
report
...
#### AFQT
This result does not fit as well as the previous two. Because we did not do imputation for AFQT.
```{r}
clean <- clean %>%
  mutate(HS_lowAFQT = ifelse(HS_90 == 1 & lowAFQT == 1, 1, 0),
         Pre_lowAFQT = ifelse(Pre_90 == 1 & lowAFQT == 1, 1, 0),
...
### IPW
In general, the result from IPW fits well for col1-3, but not col4-5. It is probably because IPW is unstable, especially when we are using so many covariates.

#### Table3 Col1
The result for col1 looks fine
```{r}
## Group 5-6 Effect
#clean_long <- clean_long %>%  mutate(None_90 = 1 - HS_90 - Pre_90)
...
cato <- c('HS_90','Pre_90','None_90')
ace.ipws <- c()
for(x in cato){
z <- Temp[[x]]
pscore <- glm(z ~ factor(Year) + factor(AgeTest_Yr) + Male , family = binomial, Temp)$fitted.values
ace.ipw = mean ( z * y / pscore ) / mean ( z / pscore )
ace.ipws <- c(ace.ipws,ace.ipw)
}
ace.ipws[4] <- ace.ipws[1] - ace.ipws[3]
ace.ipws[5] <- ace.ipws[2] - ace.ipws[3]
round(ace.ipws,3)
```
...
Temp <- clean_long %>%
  filter(Group == 2,
         !is.na(HealthCond_before+logBW+LogInc_0to3+
                  FirstBorn+
       Age2_Yr104+GMom_0to3+MomCare+RelCare+
       Moth_Smoke_BefBirth+Breastfed+
       Moth_WeightChange+Illness_1stYr)
...
for(x in cato){
z <- Temp[[x]]
pscore <- glm(z ~ factor(Year) + factor(AgeTest_Yr) + 
                Male+
       HealthCond_before+logBW+LogInc_0to3+FirstBorn+
       Age2_Yr104+GMom_0to3+MomCare+RelCare+
       Moth_Smoke_BefBirth+Breastfed+
       Moth_WeightChange+Illness_1stYr, family = binomial, Temp)$fitted.values
ace.ipw = mean ( z * y / pscore ) / mean ( z / pscore )
ace.ipws <- c(ace.ipws,ace.ipw)
}
ace.ipws[4] <- ace.ipws[1] - ace.ipws[3]
...
#### Col4
The result does not fit very well. Probably due to the violation of overlap assumption.
```{r}
### Column 4
Temp <- clean_long %>%
  filter(Group == 2)
y <- Temp$Test_std
truncpscore <- c(0.01,0.99)
...
ace.ipw_hs = mean ( z * y / pscore ) / mean ( z / pscore );ace.ipw_hs

z <- Temp$Pre_90
pscore <- glm(Pre_90 ~ factor(Year) + factor(AgeTest_Yr) + Male+ factor(MotherID) , family = binomial, Temp)$fitted.values
pscore   = pmax(truncpscore[1], pmin(truncpscore[2], pscore))
ace.ipw_pre = mean ( z * y / pscore ) / mean ( z / pscore );ace.ipw_pre

z <- Temp$None_90
pscore <- glm(z ~ factor(Year) + factor(AgeTest_Yr) + Male+ factor(MotherID) , family = binomial, Temp)$fitted.values
pscore   = pmax(truncpscore[1], pmin(truncpscore[2], pscore))
ace.ipw_none = mean ( z * y / pscore ) / mean ( z / pscore );ace.ipw_none


ace.ipw_hs - ace.ipw_none
ace.ipw_pre - ace.ipw_none
```
